...
%calc An 
for i=1:36
    if(theta(i) ~= 0 && theta(i)~= 180*(pi/180)) 
        if( theta(i) < pi)
            An(i) = (pi*(R^2)*( (sin(theta(i) + deg5 )^2) - (sin( theta(i)- deg5 )^2)));
        end
        if ( theta(i) > pi)
            An(i) = -1*(pi*(R^2)*( ((sin(theta(i) + deg5))^2) - (sin( theta(i)- deg5) )^2) );
        end
   end
end
...
%cp graph 
Cp = (rundata(1:36,2,runNum(1)) - Pstat )/(rundata(1,2,runNum(1)) - Pstat )
figure(1)
    title('Coefficient of Pressure vs Theta')
    hold on
    xlabel('theta [rad]');
    hold on
    ylabel('Cp');
